...
var mpas = func(addr string) []byte { return mpa(addr).AsSlice() }
...
func TestUnpack(t *testing.T) {
	...
	t.Run("valid minimal", func(t *testing.T) {
		...
		noErr(t, err)
		checkFlags(t, dt, DNSTAP_TYPE|MESSAGE|MESSAGE_TYPE)
	})
	...
	t.Run("reset upon Unpack", func(t *testing.T) {
		...
		// verify fields
		expected := Message{
			Flags:     DNSTAP_TYPE | MESSAGE | MESSAGE_TYPE,
			IsQuery:   true,
	...
	t.Run("check Identity", func(t *testing.T) {
		...
		checkFlags(t, dt, DNSTAP_TYPE|MESSAGE|MESSAGE_TYPE|IDENTITY)
	})
	...
	t.Run("check replace", func(t *testing.T) {
		...
		checkFlags(t, dt, DNSTAP_TYPE|MESSAGE|MESSAGE_TYPE|DNS_MESSAGE|
			IDENTITY|
			VALID_SOCKET_PROTOCOL|
			VALID_QUERY_ADDR|VALID_RESPONSE_ADDR|VALID_RESPONSE_PORT|
			has_QUERY_TIME|has_RESPONSE_TIME|VALID_TIME)
	})
	...
	t.Run("valid, then invalid", func(t *testing.T) {
		...
		noErr(t, err)
		checkFlags(t, dt, VALID|has_RESPONSE_TIME)
		dt, err = doUnpack(t, valid, invalid)
		noErr(t, err)
		checkFlags(t, dt, DNSTAP_TYPE|MESSAGE|MESSAGE_TYPE|DNS_MESSAGE|has_RESPONSE_TIME)
	})
...
func TestUnpackIPs(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if tc.QueryAddrValid {
			flags |= VALID_QUERY_ADDR
		}
		if tc.ResponseAddrValid {
			flags |= VALID_RESPONSE_ADDR
		}
...
func TestOverflows(t *testing.T) {
	...
	dt, err := doUnpack(t, msg)
	noErr(t, err)
	checkFlags(t, dt, DNSTAP_TYPE|MESSAGE|MESSAGE_TYPE|has_QUERY_TIME|has_RESPONSE_TIME)
	if dt.QueryPort != 0 {
