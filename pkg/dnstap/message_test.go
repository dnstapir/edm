...
func TestUnpack(t *testing.T) {
	...
	t.Run("check Dnstap.Type == MESSAGE", func(t *testing.T) {
		_, err := doUnpack(t, &dnstap.Dnstap{
	...
	t.Run("valid minimal", func(t *testing.T) {
		...
		noErr(t, err)
		checkFlags(t, dt, minimalNonPanicFlags)
	})
	...
	t.Run("check Identity", func(t *testing.T) {
		...
		checkFlags(t, dt, minimalNonPanicFlags|WithIdentity)
	})
	...
	t.Run("check replace", func(t *testing.T) {
		...
		checkFlags(t, dt, minimalNonPanicFlags|
			...
			withQueryTime|withResponseTime|ValidTime)
	})
	...
	t.Run("valid, then invalid", func(t *testing.T) {
		...
		checkFlags(t, dt, minimalNonPanicFlags|
			WithDNSMessage|
			ValidQueryAddr|ValidResponseAddr|ValidQueryPort|ValidResponsePort|
			ValidTime|withResponseTime)
		dt, err = doUnpack(t, valid, invalid)
		noErr(t, err)
		checkFlags(t, dt, minimalNonPanicFlags|WithDNSMessage|withResponseTime)
	})

	t.Run("accept temporarily invalid enums", func(t *testing.T) {
		dt, err := doUnpack(t, &dnstap.Dnstap{
			Type: new(dnstap.Dnstap_Type(2)),
			Message: &dnstap.Message{
				Type:         new(dnstap.Message_Type(15)),
				QueryMessage: []byte("test"),
			},
		}, []byte{
			15<<3 | uint8(protoVARINT), 1, // dnstap.Type = Message
			14<<3 | uint8(protoLEN), 2, // dnstap.Message
		...
		checkFlags(t, dt, minimalNonPanicFlags|WithDNSMessage)
	})
	...
	t.Run("check timestamp", func(t *testing.T) {
		for _, qt := range []dnstap.Message_Type{dnstap.Message_AUTH_QUERY, dnstap.Message_AUTH_RESPONSE} {
			t.Run(dnstap.Message_Type_name[int32(qt)], func(t *testing.T) {
				...
				checkFlags(t, dt, minimalNonPanicFlags|ValidTime|withQueryTime|withResponseTime)
...
func TestReadVARINT(t *testing.T) {
	...
	// check 64th bit is zero
	checkReadVARINT(t, []byte{255, 255, 255, 255, 255, 255, 255, 255, 255, 0b00000000}, math.MaxUint64^uint64(0b1)<<63)
	// check maximum value
	checkReadVARINT(t, []byte{255, 255, 255, 255, 255, 255, 255, 255, 255, 0b00000001}, math.MaxUint64)
	// check 2...255
	for i := range uint8(254) {
		panics(t, "VARINT overflow", func() { readVARINT([]byte{255, 255, 255, 255, 255, 255, 255, 255, 255, 2 + i}) })
	}
	// test length 11
...
func TestSkipVARINT(t *testing.T) {
	...
	// check 2...255
	for i := range uint8(254) {
		panics(t, "VARINT overflow", func() { skipVARINT([]byte{255, 255, 255, 255, 255, 255, 255, 255, 255, 2 + i}) })
	}
	// test length 11
