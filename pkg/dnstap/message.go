...
const (
	...
	Valid Flag = WithDnstapType | WithMessage | WithMessageType | WithDNSMessage | ValidQueryAddr | ValidResponseAddr | ValidQueryPort | ValidResponsePort | ValidTime
)
...
func (m *Message) unpackDNSTap(data []byte, meta *meta) {
	...
	if !m.HasFlags(WithDnstapType | WithMessage | WithMessageType) {
	...
	case true:
		...
		m.setTime(hasQueryTime, meta.queryTimeSec, meta.queryTimeNSec)
	case false:
		if meta.responseMessage != nil {
			m.flags |= WithDNSMessage
		}
		m.Message = meta.responseMessage
		m.setTime(hasResponseTime, meta.responseTimeSec, meta.responseTimeNSec)
	}
}
