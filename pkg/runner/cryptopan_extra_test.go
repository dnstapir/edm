...
// know when to Purge their local Crypto-PAn cache (see TIER2OPT.md
// "Change 5"); if the generation didn't strictly advance on each rotation
...
// without error or panic. Tests construct minimisers via newDnstapMinimiser
// — which allocates an inotify instance via fsnotify.NewWatcher — and rely
// on t.Cleanup(_ = edm.Close()) for resource cleanup. If a test author
// forgets and explicitly calls Close in the body, the deferred Cleanup
// must still be safe; otherwise fsWatcher.Close would error on second
// invocation and tests would intermittently fail.
//
// Implementation note: Close() sets edm.fsWatcher = nil after a successful
// close, and the second call short-circuits on the nil check.
func TestCloseIdempotent(t *testing.T) {
	...
	// First close should succeed and release the inotify instance.
	if err := edm.Close(); err != nil {
		t.Fatalf("first Close() failed: %s", err)
	}
	if edm.fsWatcher != nil {
		t.Fatalf("fsWatcher should be nil after Close, have: %#v", edm.fsWatcher)
	}

	// Subsequent calls should be no-ops; verify they do not error and do
	// not somehow re-attach a new watcher.
	for i := range 3 {
		if err := edm.Close(); err != nil {
			t.Fatalf("Close() call %d after first close: %s", i+2, err)
		}
		if edm.fsWatcher != nil {
			t.Fatalf("fsWatcher should remain nil after repeated Close, have: %#v", edm.fsWatcher)
		}
	}
}
