...
func configUpdater(viperNotifyCh chan fsnotify.Event, edm *dnstapMinimiser) {
	...
	t := time.AfterFunc(math.MaxInt64, func() {
		...
		if !conf.DisableHistogramSender {
			...
			if edm.aggregSender.aggrecURL == nil {
...
func (edm *dnstapMinimiser) setIgnoredQuestionNames() error {
	...
	if err != nil {
		return fmt.Errorf("setIgnoredQuestionsNames: unable to load dawg file '%s': %w", conf.IgnoredQuestionNamesFile, err)
	}
...
func (edm *dnstapMinimiser) configureFSWatchers(startConf config) error {
	...
	fileToFuncs[conf.IgnoredClientIPsFile] = []func() error{edm.setIgnoredClientIPs}

	fileToFuncs[conf.IgnoredQuestionNamesFile] = []func() error{edm.setIgnoredQuestionNames}

	if !conf.DisableHistogramSender {
		fileToFuncs[conf.HTTPClientCertFile] = []func() error{edm.loadHTTPClientCert}
	}

	// MQTT can only be enabled/disabled at startup
	if !startConf.DisableMQTT {
		fileToFuncs[conf.MQTTClientCertFile] = []func() error{edm.loadMQTTClientCert}
	}
...
func (edm *dnstapMinimiser) cleanupFSWatchers() error {
	...
	for _, watchPath := range edm.fsWatcher.WatchList() {
		...
		if !watchPathInUse {
			...
			if err != nil {
				return fmt.Errorf("cleanupFSWatchers: unable to remove patch watcher '%s': %w", watchPath, err)
			}
		}
	}
	edm.fsWatcherMutex.RUnlock()
...
func loadDawgFile(dawgFile string) (dawg.Finder, time.Time, error) {
	...
	if dawgFileInfo.Size() == 0 {
		return nil, time.Time{}, fmt.Errorf("loadDawgFile: dawg file is empty '%s'", dawgFile)
	}
