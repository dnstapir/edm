...
func (edm *DnstapMinimiser) sendHistogram(ctx context.Context, prevWellKnownDomainsData *wellKnownDomainsData, labelLimit int, buf *bytes.Buffer) error {
	...
	// gather data
	startTime := intervalStartFromTimes(prevWellKnownDomainsData.startTime, prevWellKnownDomainsData.rotationTime).UTC()
	duration := prevWellKnownDomainsData.rotationTime.Truncate(time.Second).Sub(startTime.Truncate(time.Second))
	// reset shared buffer
	buf.Reset()
	// marshal histogram
	...
	// add a restrictive timeout to the sender
...
func (edm *DnstapMinimiser) histogramSender(ctx context.Context, labelLimit int) {
	...
	// retry ticker
	retryTicker := edm.deps.Clock.NewTicker(edm.deps.HistogramSenderRetryInterval)
		...
		case prevWellKnownDomainsData, ok := <-edm.histogramWriterCh:
			...
			if err != nil {
				...
				case edm.histogramRetryWriterCh <- prevWellKnownDomainsData:
					edm.log.Error("histogramSender", "error", err.Error(), "retry-queued", true)
				default:
					edm.log.Error("histogramSender", "error", err.Error(), "retry-queued", false)
				}
			}
		case <-retryTicker.C():
			// handle retry queue
		retryLoop:
			for {
				...
				case prevWellKnownDomainsData := <-edm.histogramRetryWriterCh:
					...
					err := edm.sendHistogram(ctx, prevWellKnownDomainsData, labelLimit, &buf)
					if err != nil {
						edm.log.Error("histogramSender.retry", "error", err.Error())
					}
				default:
